...
export class EmployeeModuleService {
  ...
  async callYourStoredProcedure(param1: any): Promise<any> {
    const result = await this.empTypeRepository.query('CALL GetEmployeeTypeBycompany(?,?)', [param1,1]);
    return result;
}
  ...
  async getDesignation(id) {
    ...
    const result = await this.empTypeRepository.query('CALL GetDesignationBycompany(?,?)', [id,1]);
