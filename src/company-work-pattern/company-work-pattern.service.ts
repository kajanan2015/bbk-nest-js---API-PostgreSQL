...
export class CompanyWorkPatternService {
  ...
  async findCurrentWorkPattern(empId,date) {
    ...
    const patternRounds = await this.employeeassigninforepo.find({ where: { assignpatternId: workPattern?.['assign_id'], pattern_round: workpatternInfo?.['pattern_round'], status:"active" } });
console.log(patternRounds,90909)
    return {
      ...
      endDate: workPattern?.['ended_at']
    };

  }

  // ** Fine employee future work pattern
  async findFutureWorkPatterns(empId,date) {
    ...
    for (let i = 0; i < workPatterns?.length; i++) {
      // **  pattern rounds
      const patternRounds = await this.employeeassigninforepo.find({ where: { assignpatternId: workPatterns?.[i]?.['assign_id'], pattern_round: 0,status:"active"} });
      ...
      results.push({
        ...
        endDate: workPatterns?.[i]?.['ended_at']
      })
    }
...
console.log(findcurrentdata,5465445)
    ...
    if (data.availabilityfuturepattern) {
      ...
      enddateofassign=format(
        sub(new Date(availableexitpattern.assign_at), {
          days: 1,
        }),
        'yyyy-MM-dd'
      )
      lastDateAfterTwoYears = endOfDay(sub(new Date(availableexitpattern.assign_at), {
        days: 1,
      }));
    } else {
      ...
      let updateassignpatterndata;
      for (const i of existassignpattern) {
        updateassignpatterndata = {
          status: AssignWorkPatternSatatus.INACTIVE,
          updated_at: data.userTime,
          updated_by: data.created_by
        }
        await this.employeeassignrepo.update(i.assign_id, updateassignpatterndata)
      }
      // end date after 2 years
      lastDateAfterTwoYears = endOfDay(addYears(startmaindate, 2));
    }
